...
export const rule = {
  ...
  getEntity: {
    ...
    run: function (performer, type, options = {}, dimension = null) {
      ...
      switch (type) {
        ...
        case "target": {
          returnValue =
            ServerWorld.getDimension(dimension).getEntities(options)[0];
          break;
        }
        case "player": {
          options = {
          ...
          returnValue =
            ServerWorld.getDimension(dimension).getEntities(options);
  ...
  getEntities: {
    ...
    run: function (performer, options, dimension = null) {
      if (dimension === null) dimension = performer.dimension.id;
      var entities = [];
      for (const i of ServerWorld.getDimension(dimension).getEntities(options))
        entities.push(i.id);
