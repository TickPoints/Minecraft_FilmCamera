    ...
    for (let param_index = 0; param_index < configLength; param_index++) {
        ...
        if (parameters[param_index] === undefined) {
            if (requirement === "select") {
                ...
                printError(performer, "filmcamera.scripts.command_system.error.parameter_missing", param_index + 1);
                return -1;
            }
        }
        ...
        const returnValue = type_map[param.type](performer, parameters, param_index, {
            rule,
            config
        });
...
function run(command, performer = ServerWorld) {
    ...
    for (let i = 0; i < commandLength; i++) {
        if (entry === undefined) {
            printError(performer, "filmcamera.scripts.command_system.error.command_does_not_exist", command[i]);
            return;
        }
        switch (entry.type) {
            case "function": {
                ...
                if (config !== undefined) {
                    if (configureParser(config, parameters, performer) === -1) return;
                }
            ...
            if (char === "(") {
                if (com[charValue + 1] === "&") {
                    stack.push({
                        key: charValue,
                        value: "&",
                        isOuter: stack.every((item) => item.value !== "&" && !item.isOuter)
                    });
                } else {
                    stack.push({
                        key: charValue,
                ...
                const pop = stack.pop();
                if (!stack.every((item) => item.value !== "&" && !item.isOuter)) continue;
                let newCommand;
                if (pop.value === "&") {
