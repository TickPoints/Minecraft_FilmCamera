...
ServerWorld.afterEvents.playerSpawn.subscribe(eventData => {
    ...
    if (JSON.stringify(DataObject.Player[player.name]) == "{}")
        mergeData(DataObject.Player[player.name], playerDataInit());
});
...
function WorldLoad() {
    ...
    if (JSON.stringify(DataObject.World) == "{}")
        mergeData(DataObject.World, worldDataInit());
    for (const player of ServerWorld.getAllPlayers()) {
        DataObject.Player[player.name] = readDataOnDynamicProperty(player);
        if (JSON.stringify(DataObject.Player[player.name]) == "{}")
            mergeData(DataObject.Player[player.name], playerDataInit());
...
export { getDataManager, saveData, writeWorldDataInit, writePlayerDataInit };
